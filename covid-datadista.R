...
calcular_incidencia <- function(datos, dias, poblacion) {
    ...
    for (i in dias:(nrow(datos)))
    {
        ...
        for (j in 0:(dias-1))
        {            
            incidencia <- incidencia + datos[i-j, "Casos"]
        }
...
calcular_diferencia <- function(datos) {
    diferencia <- rep(0, nrow(datos))
    for (i in 2:(nrow(datos)))
    {
        diferencia[i] <- datos$ia14d[i] - datos$ia14d[i-1]
    }
    result <- diferencia
}
...
datos_calculados$DIF_CLM = calcular_diferencia(datos_calculados)
...
datos_calculados$DIF_ES = calcular_diferencia(datos_es)
